<script lang="ts" context="module">
  // 移动端专用副本：与 DatabaseObjectAppObject.svelte 保持一致，唯一差异是实例模板
  // 用 display:contents 包裹层监听 pointerup 直接打开 tab（移动端 click 合成不可靠，
  // 见 handleObjectRowPointerUp 注释）；原 DatabaseObjectAppObject.svelte 保持原样。
...
<script lang="ts">
  ...
  // 展开箭头/pin/status 图标的 click 有 stopPropagation，但 pointerup 仍会冒泡到包裹层，
  // 因此显式过滤。
  function handleObjectRowPointerUp(e) {
    if (e.button !== 0) return;
    if (e.target.closest('.expand-icon, .pin, .pin-active, .unpin, .status')) return;
    handleObjectClick(data, 'leftClick');
  }
...
<div style="display: contents" on:pointerup={handleObjectRowPointerUp}>
  <AppObjectCore
    {...$$restProps}
    module={$$props.module}
    {data}
