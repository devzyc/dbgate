<script lang="ts">
  // 移动端专用副本：与 ConnectionList.svelte 保持一致，
  // 仅将 on:click 改为 on:pointerup（移动端可靠），
  // 确保触摸点击能正确触发新建连接等操作。
  import _ from 'lodash';
  import MbInlineButton from '../buttons/MbInlineButton.svelte';
  ...
  import { openTabRequestSignal } from '../mbStores';
  import {
    commandsCustomized,
    expandedConnections,
    openedConnections,
  ...
  import MbLargeButton from '../buttons/MbLargeButton.svelte';
...
<SearchBoxWrapper {filter}>
  <SearchInput
    ...
    data-testid="MbConnectionList_search"
  />
  ...
  {#if $commandsCustomized['new.connection']?.enabled}
    <MbInlineButton
      on:pointerup={() => {
        openTabRequestSignal.update(x => x + 1);
        runCommand('new.connection');
      }}
      title={_t('connection.new.title', { defaultMessage: 'Add new connection' })}
      data-testid="MbConnectionList_buttonNewConnection"
    >
      <FontIcon icon="icon plus-thick" />
    </MbInlineButton>
    <MbInlineButton
      on:pointerup={() => runCommand('new.connection.folder')}
  ...
  {/if}
  <MbInlineButton
    on:pointerup={handleRefreshConnections}
...
<WidgetsInnerContainer
  ...
  data-testid="MbConnectionList_container"
>
  ...
  {#if $connections && $connections.length == 0 && $openedConnections.length == 0 && $commandsCustomized['new.connection']?.enabled && !$openedTabs.find(x => !x.closedTime && x.tabComponent == 'ConnectionTab' && !x.props?.conid)}
    <MbLargeButton
      ...
      fillHorizontal
      >Add new connection</MbLargeButton
    >
  {/if}
</WidgetsInnerContainer>
